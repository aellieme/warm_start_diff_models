...
# Импорт из существующих файлов (не изменяем их)
...
# Настройка логирования для Optuna (будет выводиться в консоль)
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

def compute_recall10_on_validation(model, val_loader, args):
    """
    recall@10 на валидационном наборе для заданной модели, нужен
    после обучения для оценки качества гиперпараметров
    """
    device = args.device
    model.eval()
    all_actual = []
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            _, topk = torch.topk(scores, k=10, dim=-1)   # только k=10
    ...
    return recalls[0]   # Recall@10

def objective(trial, base_args, tra_data_loader, val_data_loader, test_data_loader):
    """
    Целевая функция для Optuna.
    Предлагает гиперпараметры, обучает модель и возвращает Recall@10 на валидации.
    """
    # базовый конфиг
    ...
    # Копируем базовые параметры и обновляем гиперпараметрами
    ...
    # Уменьшаем количество эпох для тюнинга 
    args.epochs = 20
    ...
    # Пересоздаём загрузчики с новым batch_size 
    tra_data = Data_Train(tra_data_loader.u2seq, args)  
    val_data = Data_Val(val_data_loader.u2seq, val_data_loader.u2answer, args)
    test_data = Data_Test(test_data_loader.u2seq, test_data_loader.u2seq_add, test_data_loader.u2answer, args)

    tra_loader = tra_data.get_pytorch_dataloaders()
    val_loader = val_data.get_pytorch_dataloaders()
    test_loader = test_data.get_pytorch_dataloaders()   # нужен для model_train
...
def main():
    ...
    tra_data = Data_Train(data_raw['train'], base_args)
    val_data = Data_Val(data_raw['val_seq'], data_raw['val'], base_args)
    test_data = Data_Test(data_raw['test_seq'], {uid: [] for uid in data_raw['test_seq']}, data_raw['test'], base_args)
    ...
    objective_partial = partial(objective, base_args=base_args,
                                tra_data_loader=tra_data,
                                val_data_loader=val_data,
                                test_data_loader=test_data)
    study.optimize(objective_partial, n_trials=15, timeout=3600*1)  # 15 trials или 1 час
    ...
    # Пересоздаём загрузчики с финальным batch_size
    ...
    # Baseline и adaptation инференс
    print("\n Final evaluation ")
    baseline_test_seq = {}
