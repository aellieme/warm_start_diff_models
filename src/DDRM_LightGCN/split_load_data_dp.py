...
class GlobalTemporalSplitter:
    ...
    Разделение:
      ...
      3. Adapt   (train_p+val_p – train_p+val_p+adapt_p)
      4. Test    (оставшиеся)
    """
    ...
    def split(self, train_p=0.7, val_p=0.1, adapt_p=0.1):
        ...
        adapt_cutoff = time_values.quantile(train_p + val_p + adapt_p)

        print(f"[Cutoffs] Train ≤ {train_cutoff}, Val ≤ {val_cutoff}, Adapt ≤ {adapt_cutoff}")
        ...
        adapt_df = self.df[(self.df[self.time_col] > val_cutoff) & (self.df[self.time_col] <= adapt_cutoff)].copy()
        test_df  = self.df[self.df[self.time_col] > adapt_cutoff].copy()
    ...
    def save_splits(self, data_dict, output_path):
        ...
        files = {
            ...
            'adapt_list.npy': data_dict['adapt'],
