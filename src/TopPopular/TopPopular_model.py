...
# ========== 1. Фиксация случайного seed = 42 (для воспроизводимости) ==========
random.seed(42)
np.random.seed(42)
...
# ========== 3. Популярность считается ТОЛЬКО на train (без val и adapt) ==========
popularity_scores = train_df['movieid'].value_counts().index.tolist()

# Пользователи теста (только те, у кого есть взаимодействия в test_df)
...
# ========== 4. Запуск инференса (только baseline) ==========
TOP_K_LIST = [1, 10, 20, 50, 100]
print("\nBaseline Inference (train+val → test)")
...
# Вычисляем метрики для всех K за один вызов (preds_baseline_final содержит top-max_K)
(_, recalls_baseline, ndcgs_baseline, mrrs_baseline, covs_baseline) = compute_all_metrics(
    test_grouped, preds_baseline_final, TOP_K_LIST, all_items_count
)

print_results("BASELINE (Top-Popular)", TOP_K_LIST,
              recalls_baseline, ndcgs_baseline, mrrs_baseline, covs_baseline,
